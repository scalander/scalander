function calendarPaddingHelper(month, year, mondayWeeks = false) {
    ...
    // we want to output 6 rows consisntently
    // so if the padding isn't >7, we will make it so
    // by adding another row of 7

    if ((beginPaddingDates.length + endPaddingDates.length) <= 7) {
        // we will count from either the lastDay of the month
        // if the month needed no padding or the last day
        // of padding
        let countFrom = (endPaddingDates.length==0) ? lastDay :
            endPaddingDates[endPaddingDates.length-1];

        // we now add a week to padding
        for (let pad = 1; pad <= 7; pad++) {
            endPaddingDates.push(new Date(countFrom.getFullYear(),
                                          countFrom.getMonth(),
                                          countFrom.getDate()+pad));
        }
    }
